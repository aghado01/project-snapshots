...
Each pair of data points is measured with a caller-chosen metric: `Hamming` (on 64-bit SimHash signatures), `Euclidean`, `Canberra`, `Manhattan`, `Minkowski` (configurable _p_), `Cosine`, `JensenShannon` (square root), `Jaccard` (threshold-binarised), `Wasserstein` (1D Earth Mover's), `Mahalanobis` (caller-supplied Σ⁻¹), `FisherRao` (geodesic on the statistical manifold), or `Poincare` (hyperbolic unit-ball distance).
...
artifacts/
├── bin/    # Build output for all projects (generated; not source)
└── obj/    # Intermediate build objects (generated; not source)

projects/
├── LinearAlgebra/LinearAlgebra.csproj # Standalone Cholesky decomposition primitive (no deps)
├── GaussianMixture/GaussianMixture.csproj # GMM: EM fitting, sampling, density, Mahal; depends on LinearAlgebra + DistanceMetrics
...
├── VizCore/VizCore.csproj # VizCore layer model: scene pipeline, adapter, JSON render target
├── VizApi/VizApi.csproj # ASP.NET Core Minimal API: POST /api/regen → ScenePackage JSON
...
│ ├── EuclideanMedian.cs # Weighted Euclidean geometric median
│ ├── GeometricMean.cs # Riemannian geometric mean (manifold-aware)
│ ├── GeometricMedian.cs # Riemannian geometric median
│ ├── KarcherScatter.cs # Karcher/Fréchet scatter matrix
│ ├── ScatterAccumulator.cs # Accumulation primitives for scatter estimation
│ ├── WeiszfeldScatter.cs # Weiszfeld-based scatter estimation
│ └── ConsistencyFactors.cs # Bias-correction factors for robust scatter estimates
│
...
├── CrescentEllipsoid.cs # Crescent arc + anisotropic ellipsoid; configurable arc noise, placement, optional 4D
└── MobiusEllipsoid.cs # Möbius tube + anisotropic ellipsoid; configurable cross-section, placement, optional 4D lift

├── viz-core/ # VizCore + VizApi source files
│ ├── viz_core.cs # Layer model: PointCloud, LabelLayer, ScalarLayer, EdgeLayer, GaussianLayer, SpineLayer, VizDataset
│ ├── adapter.cs # SyntheticDatasetAdapter: dataset → VizDataset layer graph
│ ├── scene_renderer.cs # SceneBuilder, SceneDescriptor, ScenePackage
│ ├── html_render_target.cs # ThreeJsHtmlRenderTarget (static HTML + viewer.html)
│ ├── serializer.cs # JSON serialization for ScenePackage (JsonExportRenderTarget)
│ ├── param_schema.cs # ParamSchema / ParamSection / ParamEntry DTOs
│ ├── schema_catalog.cs # KnownGenerators, generator schemas (Crescent+Ellipsoid, Möbius+Ellipsoid)
│ └── viewer.html # Three.js r160 interactive viewer: regen panel, edge/GMM/spine toggles, semantic PALETTE
...
```powershell
dotnet build .\projects\LinearAlgebra\LinearAlgebra.csproj
dotnet build .\projects\GaussianMixture\GaussianMixture.csproj
...
dotnet build .\projects\VizCore\VizCore.csproj
dotnet build .\projects\VizApi\VizApi.csproj
dotnet build .\projects\VizCoreSmoke\VizCoreSmoke.csproj
...
The `estimators/` folder is compiled as `StatisticalEstimators`. It is the application-agnostic statistical-estimation layer for reusable computational primitives:

- **Euclidean estimators** — `EuclideanMean` (weighted Fréchet mean), `EuclideanMedian` (weighted geometric median)
- **Riemannian estimators** — `GeometricMean` (manifold-aware Karcher mean), `GeometricMedian` (Riemannian geometric median)
- **Scatter estimators** — `KarcherScatter` (Karcher/Fréchet scatter matrix), `ScatterAccumulator` (accumulation primitives), `WeiszfeldScatter` (Weiszfeld-based scatter)
- **Bias correction** — `ConsistencyFactors` (bias-correction factors for robust scatter estimates)
