...
> **Status:** DRAFT v0.2 — Phase 2 implemented (VizCore layer model + Three.js Pass 1)
> **Date:** 2026-05-06
...
| **Topology** | Build / threshold the proximity graph from a distance source               | Distance matrix or KNN list, graph rule, k or ε      | Edge list with weights                               |
| **Render**   | Draw point clouds, edges, ellipsoids, traces, dendrograms                  | Edge list, point coordinates, GMM parameters, traces | Frames on a canvas                                   |

Layer 1 is `DistanceMetrics` + tangent-frame helpers; layer 2 is `ProximityGraphs`; layer 3 is the only piece that is genuinely new code (a renderer + control surface). Layers 1 and 2 are shared with the headless engine — there is no separate visualization-only metric or graph rule.
...
2. **[DONE] Synthetic dataset rendering.** `VizCore` layer model (`PointCloud`, `LabelLayer`, `ScalarLayer`, `EdgeLayer`, `GaussianLayer`, `SpineLayer`, `TemporalLabelSequence`, `VizDataset`), `SceneDescriptor` / `ScenePackage` / `SceneBuilder` pipeline, `SyntheticDatasetAdapter`, and `ThreeJsHtmlRenderTarget` Pass 1 (point cloud + GT label coloring, OrbitControls, auto-fit, cluster legend). Projects: `VizCore`, `VizCoreSmoke`. Smoke test confirmed: `GenerateCrescentAndEllipsoid` → adapter → scene builder → HTML file renders correctly in browser.
3. **Distance + graph layer.** Render edges; add metric / graph / k controls.
4. **Tangent-frame overlay** on synthetic generators that expose an analytic frame.
...
6. **GMM handoff visualisation.** Render core points (coherence-weighted), initial ellipsoids, and (recursive case) the `ComponentToClusterMap`-coloured tiling.
7. **Configuration-matrix dashboard.** Wire all controls together; produce the first end-to-end case study.
8. **Live compute path (Wasm).** Compile C# primitives, expose a sandbox page.
...
> **Next steps:** Phase 3 — edge rendering for proximity graphs (`EdgeLayer` pass in `ThreeJsHtmlRenderTarget`). Requires `DistanceMetrics` + `ProximityGraphs` project references added to `VizCore.csproj` and a diagnostic harness that populates `EdgeLayer` with `EdgeClusterSrc`/`EdgeClusterDst` for false-bridge highlighting.

Phases 5–7 land alongside the merge-tree and cluster-profile codecs in the state engine.
