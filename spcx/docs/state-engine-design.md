...
- **Hierarchical / recursive GMM codecs** — `cluster/tree-node/v1` (per-node parameters + `ManifoldCenter` + path-encoded `NodeId`) and `cluster/tree-manifest/v1` (topology) are reserved for the recursive-mixture variant tracked in [gmm-maturity-extentions.md](./gmm-maturity-extentions.md). Path-encoded node IDs (`"root.0.1"`) carry resume identity. Adds a `HierarchicalGmm` lifecycle stage when the work lands; not v1.
- **Shared partition-cutting layer** — interfaces (`IPartitionSequence<TScore>`, `IPartitionCriterion<TScore>`, `CutResult`, `IClusterMembership`) are shared between SPC and GMM, so they live in a future `Clustering` primitive rather than in either application stack. The codec inventory consumes the interfaces from both stacks; the engine itself is unaffected. See [project-primer.md](./project-primer.md) and [spc-maturity.md](./spc-maturity.md) §17.6.
