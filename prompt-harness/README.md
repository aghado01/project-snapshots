...
```
python -m src.runners.batch --config configs/providers.yaml --prompts data/prompts --out reports/test-run --provider echo --model echo-small --seed 42
```
...
python -m src.runners.batch --out reports/candidate-test --prompts data/prompts --config configs/providers.yaml
...
scripts/run_baseline.ps1         # Baseline evaluation (creates reports/baseline-latest)
python -m src.runners.batch --out reports/candidate --prompts data/prompts
pwsh scripts/compare_gates.ps1 -Candidate reports/candidate
```
